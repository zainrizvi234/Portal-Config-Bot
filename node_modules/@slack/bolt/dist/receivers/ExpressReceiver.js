...
function verifySignatureAndParseRawBody(logger, signingSecret, bodyLimit) {
 ...
 */
function buildVerificationBodyParserMiddleware(logger, signingSecret, bodyLimit) {
...
function buildBodyParserMiddleware(logger, bodyLimit) {
    return async (req, res, next) => {
        let stringBody;
        try {
            stringBody = await parseExpressRequestRawBody(req, bodyLimit);
...
async function parseExpressRequestRawBody(req, bodyLimit) {
    ...
    return (await (0, raw_body_1.default)(req, rawBodyOptions)).toString();
}
